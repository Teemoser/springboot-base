...
public enum ResultStatusEnum {
    ...
    //1000以内是系统错误，
    no_login(1000,"未登录"),
    login_time_out(1001,"登录失效"),
    config_error(1002,"参数配置表错误"),
    user_exist(1003,"用户名已存在"),
    userpwd_not_exist(1004,"用户名不存在或者密码错误"),
    client_error(1005,"身份来源异常");
    ...
    public static int getCode(String msg){
        try{
            return ResultStatusEnum.valueOf(msg).code;
        }catch (IllegalArgumentException e){
            log.error("Can't found error code: {}", msg);
            return FAIL.getErrorCode();
        }
    }

    public static String getMsg(int code) {
        for(ResultStatusEnum res:ResultStatusEnum.values()){
            if (res.code == code){
                return res.msg;
            }
        }
    ...
    public static String getMsg(String msg) {
        try{
            return  ResultStatusEnum.valueOf(msg).msg;
        }catch (IllegalArgumentException e){
            log.error("Can't found error code: {}", msg);
            return "Error Code not found";
        }
